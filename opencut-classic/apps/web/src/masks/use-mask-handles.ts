...
import { useEditor, useMedia, usePlayback, useSelection, useTimeline } from "@/editor/use-editor";
...
export function useMaskHandles({
	...
	const currentTime = usePlayback((e) => e.playback.getCurrentTime());
	...
	const selectedMaskPointSelection = useSelection((e) =>
		e.selection.getSelectedMaskPointSelection(),
	);

	const elementsWithBounds = useMemo(
		() => getVisibleElementsWithBounds({ tracks, currentTime, canvasSize, mediaAssets }),
