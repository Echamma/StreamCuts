...
export function EditorHeader() {
	...
	const handleOpenExportsFolder = async () => {
		if (isOpeningExportsFolder) return;

		setIsOpeningExportsFolder(true);

		try {
			const result = await openExportsFolder();
			toast.success("Opened export folder.", {
				description: result.folderPath,
			});
		} catch (error) {
			toast.error("Could not open export folder", {
				description:
					error instanceof Error ? error.message : "Please try again.",
			});
		} finally {
			setIsOpeningExportsFolder(false);
		}
	};
		...
		<header className="bg-background flex h-[3.4rem] items-center justify-between px-3 pt-0.5">
			...
			<nav className="flex items-center gap-2">
				...
				<Button
					...
					disabled={isOpeningExportsFolder}
					className="gap-1.5"
				>
					<HugeiconsIcon icon={Folder03Icon} className="size-4" />
					{isOpeningExportsFolder ? "Opening..." : "Open folder"}
				</Button>
				<ExportButton />
				<ThemeToggle />
			</nav>
