...
export class PreviewInteractionController {
	...
	constructor({ depsRef }: { depsRef: PreviewInteractionDepsRef }) {
		...
		this.beginTextEdit = this.beginTextEdit.bind(this);
	...
	beginTextEdit({ trackId, elementId, element }: EditingTextState): void {
		if (this.editingTextState || this.deps.preview.isMaskMode()) return;
		this.editingTextState = { trackId, elementId, element };
		this.notify();
	}
