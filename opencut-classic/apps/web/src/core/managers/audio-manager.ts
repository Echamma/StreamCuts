...
export class AudioManager {
	...
	private handleTimelineChange = (): void => {
		...
		// When no scene is loaded (e.g. during project creation before initializeScenes
		// runs) treat the active set as empty so all cached state is evicted.
		const activeKeys = activeScene
			? this.getActiveSourceKeys({ tracks: activeScene.tracks, mediaAssets })
			: new Set<string>();

		// Evict sinks and inputs whose source is no longer in the project.
		// Sinks for sources still in use are retained so the next playback session
		// can reuse them without re-opening / re-seeking.
		...
		// Retain decoded buffers for sources still in the project — decoding an
		// entire audio file is the most expensive step (full demux + decode).
		// Evict only buffers whose source was removed or replaced.
		...
		// Prepared clip buffers embed trim/duration/retime, so they must be
		// regenerated after any timeline change.  They are cheap to rebuild
		// because they only slice/resample an already-decoded buffer.
		this.preparedClipBuffers.clear();

		// Kill iterators that reference stale clip positions.
		this.stopPlayback();
