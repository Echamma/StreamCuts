...
import { SessionViewStateStore } from "./session-view-state-store";
...
export class ProjectManager {
	...
	private readonly sessionViewStateStore = new SessionViewStateStore();
	...
	async loadProject({ id }: { id: string }): Promise<void> {
		...
		try {
			...
			const sessionViewState = await this.sessionViewStateStore.load({
				projectId: id,
				fallback: project.timelineViewState,
			});

			this.active = {
				...project,
				timelineViewState: sessionViewState,
			};
			this.notify();
	...
	async export({ options }: { options: ExportOptions }): Promise<ExportResult> {
		...
		this.exportState = { isExporting: true, progress: 0, phase: "idle", statusText: "", result: null };
		this.notify();

		const sceneTarget = options.sceneTarget ?? { mode: "current" };
		let tracksOverride: SceneTracks | undefined;
		const originalSceneId = this.editor.scenes.getActiveSceneOrNull()?.id;

		if (sceneTarget.mode === "specific") {
			await this.editor.scenes.switchToScene({ sceneId: sceneTarget.sceneId });
		} else if (sceneTarget.mode === "all") {
			const allScenes = this.editor.scenes.getScenes();
			tracksOverride = mergeSceneTracks({ scenes: allScenes }).tracks;
		}

		const result = await this.editor.renderer.exportProject({
			options,
			tracks: tracksOverride,
			onProgress: ({ progress, phase, statusText }) => {
				this.exportState = {
					...this.exportState,
					progress,
					phase: phase ?? "video",
		...
		if (sceneTarget.mode === "specific" && originalSceneId) {
			await this.editor.scenes.switchToScene({ sceneId: originalSceneId });
		}
	...
	clearExportState(): void {
		this.exportState = { isExporting: false, progress: 0, phase: "idle", statusText: "", result: null };
		this.notify();
	}
	...
	closeProject(): void {
		this.sessionViewStateStore.destroy();
		this.active = null;
		this.notify();
	...
	setTimelineViewState({ viewState }: { viewState: TTimelineViewState }): void {
		...
		this.sessionViewStateStore.set({
	...
	setTranscript({ transcript }: { transcript: SavedTranscript }): void {
		if (!this.active) return;
		this.active = { ...this.active, transcript };
		this.notify();
		this.editor.save.markDirty();
	}

	clearTranscript(): void {
		if (!this.active || !this.active.transcript) return;
		const { transcript: _t, ...rest } = this.active;
		this.active = rest as TProject;
		this.notify();
		this.editor.save.markDirty();
	}
