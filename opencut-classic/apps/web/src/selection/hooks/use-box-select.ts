...
export function useBoxSelect<TId>({
	...
	useEffect(() => {
		...
		const latestPos = { x: selectionBox.startPos.x, y: selectionBox.startPos.y };
		...
		const handleMouseMove = ({ clientX, clientY }: MouseEvent) => {
			...
			rafId = requestAnimationFrame(() => {
				rafId = null;
				const currentPos = { x: latestPos.x, y: latestPos.y };
				const deltaX = Math.abs(currentPos.x - selectionBox.startPos.x);
				const deltaY = Math.abs(currentPos.y - selectionBox.startPos.y);
				const container = containerRef.current;
