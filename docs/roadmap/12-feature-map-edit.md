...
| ID | Resolve feature | Status | StreamCuts anchor | Proposed approach | Feasibility | Effort | Phase |
|---|---|---|---|---|---|---|---|
| EDIT-005 | Markers with notes, colors, durations | MISSING — only scene `bookmarks` | `timeline/types.ts` `Bookmark`, ruler UI | Extend bookmarks → markers `{color, note, duration?, elementId?}` (timeline + clip markers); marker list panel; storage migration | browser-native | M | 1 |
